...
class ProjectManagementService
{
	...
	public static function getProjects($user = null, $userFilter = null)
	{
		...
		// project owner
		$filterPjOwner = null;
		if (isset($_GET['pj_owner']) && $_GET['pj_owner'] != "" && $_GET['pj_owner'] != "undefined") {
			$filter = true;
			if ($where == "") {
				$where = " WHERE ";
			} else {
			...
			$join .= " JOIN p.owner ow ";
			$filterPjOwner = $_GET['pj_owner'];
			$where .= " (ow.email LIKE :pj_owner ) ";
		}

		// project manager
		$filterPjManager = null;
		if (isset($_GET['pj_manager']) && $_GET['pj_manager'] != "" && $_GET['pj_manager'] != "undefined") {
			$filter = true;
			if ($where == "") {
				$where = " WHERE ";
			} else {
			...
			$join .= " JOIN p.analystInCharge mgr ";
			$filterPjManager = $_GET['pj_manager'];
			$where .= " (mgr.email LIKE :pj_manager ) ";
		}

		// project manager
		$filterPjAnalysts = null;
		if (isset($_GET['pj_analysts']) && $_GET['pj_analysts'] != "" && $_GET['pj_analysts'] != "undefined") {
			$filter = true;
			if ($where == "") {
				$where = " WHERE ";
			} else {
			...
			$join .= " JOIN p.analystsInvolved invo ";
			$filterPjAnalysts = $_GET['pj_analysts'];
			$where .= " (invo.email LIKE :pj_analysts ) ";
		}
		...
		if (isset($_GET['mth_pf']) && $_GET['mth_pf'] != "" && $_GET['mth_pf'] != "undefined" && $_GET['mth_pf'] != "null") {
			...
			// split PF
			$listPF = preg_split('/,/', $_GET['mth_pf']);
			foreach ($listPF as &$value) {
				// add filter value
				$filterMthPF_value = intval(preg_split('/_/', $value)[2]);
				$filterMthPF[$filterMthPFcount] = $filterMthPF_value;
				if ($filterMthPFcount > 0) {
					$where .= " AND ";
				}
				// join plateform
				$join .= " JOIN p.mthPlatforms pf_$filterMthPFcount ";
				// add to where clause
				$where .= " (pf_$filterMthPFcount.id = :mthPF_$filterMthPFcount ) ";
				$filterMthPFcount++;
			}
		...
		if ($filter) {
			...
			if (!is_null($filterPjOwner)) {
				$queryParam['pj_owner'] = "%" . $filterPjOwner . "%";
			}
			// project manager filter
			if (!is_null($filterPjManager)) {
				$queryParam['pj_manager'] = "%" . $filterPjManager . "%";
			}
			// project involved filter
			if (!is_null($filterPjAnalysts)) {
				$queryParam['pj_analysts'] = "%" . $filterPjAnalysts . "%";
			}
			// date filter
			...
			// filter on plateforms (AND linker)
			if (!is_null($filterMthPF) && !empty($filterMthPF)) {
				for ($i = 0; $i < $filterMthPFcount; $i++) {
					$queryParam["mthPF_$i"] = "" . $filterMthPF[$i] . "";
				}
			}

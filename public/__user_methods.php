...
$app->put('/user[/{id}]', function ($request, $response, $args) {
	...
	// mama#41
	(isset($putData["mthPlatform"]) && $putData['mthPlatform'] != "") ? $mthPlatform = intval($putData["mthPlatform"]) : $mthPlatform = null;

	if ($data) {
		$data = UserManagementService::update($id, null, $firstName, $lastName, $phoneGroup, $phoneNumber, $laboratoryOrCompagny, $workplaceAddress, $typeOfLaboratoryOrCompany, $emailReception, $emailAlertNewUserAccount, $emailAlertNewProject, $emailAlertNewEventFollowedProject, $emailAlertNewMessage, $mthPlatform, $emailLanguage, $currentUser);
	}

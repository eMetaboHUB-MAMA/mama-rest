...
// mama#66 - new manager keywords

$app->get('/managerkeywords', function ($request, $response, $args) {
	// init response
	$data = KeywordManagementService::getManagerKeywords();
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [
			"managerkeywords" => $data
		];
	}
	return formatResponse($request, $response, $args, $data);
});

$app->post('/managerkeyword', function ($request, $response, $args) {
	if (!isAdmin()) {
		return formatResponse401($request, $response);
	}
	$user = TokenManagementService::getUserFromToken(getToken());
	$mthPlatform = $_POST['keyword'];
	$data = KeywordManagementService::createManager($mthPlatform, $user);
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [
...
$app->put('/managerkeyword[/{id}]', function ($request, $response, $args) {
	$id = intval($args['id']);
	if (!isAdmin()) {
		return formatResponse401($request, $response);
	}
	...
	$data = KeywordManagementService::updateManager($id, $mthPlatform, $deleted, $user);
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [
...
// mama#65
$app->get('/mth-sub-platforms', function ($request, $response, $args) {
	// admin or manager
	if (!(isProjectManager() || isAdmin())) {
		return formatResponse401($request, $response);
	}
	// init response
	$data = MTHSubPlatformManagementService::getMTHSubPlatforms();
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [
...
$app->post('/mth-sub-platform', function ($request, $response, $args) {
	if (!isAdmin()) {
		return formatResponse401($request, $response);
	}
	$user = TokenManagementService::getUserFromToken(getToken());
	$mthSubPlatform = $_POST['mthSubPlatform'];
	$data = MTHSubPlatformManagementService::create($mthSubPlatform, $user);
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [
...
$app->put('/mth-sub-platform[/{id}]', function ($request, $response, $args) {
	$id = intval($args['id']);
	if (!isAdmin()) {
		return formatResponse401($request, $response);
	}
	$putData = $GLOBALS['putData'];
	$user = TokenManagementService::getUserFromToken(getToken());
	(isset($putData["mthSubPlatform"]) && $putData['mthSubPlatform'] != "") ? $mthSubPlatform = $putData['mthSubPlatform'] : $mthSubPlatform = null;

	$data = MTHSubPlatformManagementService::update($id, $mthSubPlatform, $user);
	// special case: XML lists
	$headerValueArray = $request->getHeader('Accept');
	if (in_array("application/xml", $headerValueArray) || getFormat() == "xml") {
		$data = [

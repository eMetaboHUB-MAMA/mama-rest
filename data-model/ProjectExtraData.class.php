...
class ProjectExtraData extends AbstractMAMAobject
{
    ...
    // mama#61 - new "manager context"

    /**
     *
     * @Column(type="string", nullable=true, name="manager_context")
     *
     * @var String
     * @access private
     */
    ...
    /**
     *
     * @Column(type="boolean", name="know_mth_via_formal_user", nullable=true)
     *
     * @var Boolean
     * @access private
     */
    private $knowMTHviaFormalUser;
    ...
    // new mama#66 - thematic keyword in extra-data (only for projects manager)
    /**
     * @ManyToMany(targetEntity="ProjectExtraDataThematicCloudWord")
     * @JoinTable(name="projects_to_managers_thematic_words",
     * joinColumns={@JoinColumn(name="project_id", referencedColumnName="id")},
     * inverseJoinColumns={@JoinColumn(name="projects_managers_thematic_word_id", referencedColumnName="id")}
     * )
     * @var Array<ProjectExtraDataThematicCloudWord>
     * @access private
     */
    private $managerThematicWords;

    // mama#68 - new "ID to follow project outside of MAMA"

    /**
     *
     * @Column(type="string", nullable=true, name="external_manager_identifier")
     *
     * @var String
     * @access private
     */
    private $externalManagerIdentifier;

    // mama#65 - sub-platform

    /**
     * @ManyToMany(targetEntity="MTHsubplatform")
     * @JoinTable(name="projects_ext_data_to_metabohub_sub_platforms",
     * joinColumns={@JoinColumn(name="project_id", referencedColumnName="id")},
     * inverseJoinColumns={@JoinColumn(name="metabohub_sub_platform_id", referencedColumnName="id")}
     * )
     *
     * @var Array<MTHsubplatform>
     * @access private
     */
    private $mthSubPlatforms;
    ...
    public function __construct($analysisRequest)
    {
        ...
        // new mama#66 
        $this->managerThematicWords = new \Doctrine\Common\Collections\ArrayCollection();
        // new mama#65
        $this->mthSubPlatforms = new \Doctrine\Common\Collections\ArrayCollection();
    }
    ...
    public function getManagerContext()
    {
        return $this->managerContext;
    }

    public function setManagerContext($managerContext)
    {
    ...
    public function getKnowMTHviaFormalUser()
    {
        return $this->knowMTHviaFormalUser;
    }

    public function setKnowMTHviaFormalUser($knowMTHviaFormalUser)
    {
        $this->knowMTHviaFormalUser = $knowMTHviaFormalUser;
    }
    ...
    // mama#66
    public function getManagerThematicWords()
    {
        return $this->managerThematicWords;
    }
    public function setManagerThematicWords($managerThematicWords)
    {
        $this->managerThematicWords = $managerThematicWords;
    }

    // mama#68
    public function getExternalManagerIdentifier()
    {
        return $this->externalManagerIdentifier;
    }

    /**
     * Set the externalManagerIdentifier text value
     *
     * @param String $externalManagerIdentifier
     *            the value to set
     */
    public function setExternalManagerIdentifier(string $externalManagerIdentifier): void
    {
        $this->externalManagerIdentifier = $externalManagerIdentifier;
    }
    ...
    public function prune()
    {
        ...
        // mama#66
        $managerCloudWords = array();
        foreach ($this->managerThematicWords as $k => $v) {
            $cloudWord = [
                "id" => intval($v->getId()),
                "word" => $v->getWord()
            ];
            array_push($managerCloudWords, $cloudWord);
        }
        $this->managerThematicWords = $managerCloudWords;
        // mama#68
        $this->externalManagerIdentifier = $this->getExternalManagerIdentifier();
        if (is_null($this->externalManagerIdentifier)) {
            $this->externalManagerIdentifier = "";
        }
        // mama#65
        $platforms = array();
        foreach ($this->mthSubPlatforms as $k => $v) {
            $platform = [
                "id" => intval($v->getId()),
                "name" => $v->getName()
            ];
        ...
        $this->mthSubPlatforms = $platforms;
    }
    ...
    // mama#65
    public function getMthSubPlatforms()
    {
        return $this->mthSubPlatforms;
    }
    public function setMthSubPlatforms($mthSubPlatforms)
    {
        $this->mthSubPlatforms = $mthSubPlatforms;
    }
}
